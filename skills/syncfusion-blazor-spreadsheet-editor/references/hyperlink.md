...
| `HYPERLINK` | Specifies the destination of the hyperlink. This can be a web URL (with or without a protocol), a cell reference, or a sheet reference. |`"https://www.syncfusion.com/blazor-components/blazor-spreadsheet"`|
| `DISPLAYTEXT` | This is optional parameter. Specifies the text to display in the cell. If omitted, the hyperlink address is used as the display text. For cells with existing values, this parameter overrides the existing text. |`"Syncfusion"`|
