...
function Default() {
    ...
    //Enable the isFinite property to render only limited number of rows and columns.
    //In Finite mode, it will render the rows and columns based on the row and column count.
    // const scrollSettings = {
    //   isFinite: true,
    //   enableVirtualization: true
    // };
    const onCreated = () => {
      const spreadsheet = spresdsheetRef.current;
      // To go to specific cell.
      spreadsheet.goTo('A5000');
    }
    
    return (<SpreadsheetComponent ref={spreadsheetRef} scrollSettings={scrollSettings} created={onCreated}>
                    <SheetsDirective>
                        <SheetDirective name="Sheet1">
                          <RangesDirective>
                              <RangeDirective dataSource={localData}></RangeDirective>
                          </RangesDirective>
                        </SheetDirective>
                    </SheetsDirective>
...
function Default() {
    ...
    return (<SpreadsheetComponent ref={spreadsheetRef} scrollSettings={scrollSettings}></SpreadsheetComponent>);
}
export default Default;
```
